 ...
 *
 *  WARNING: This Driver code has NOT been tested
 *
 *  Instructions:
...
//Pure Humidify+Cool oscillation modes
def static humidificationModeMap() {["Off":"OFF","Humidify":"HUMD"]}
def static humidificationAutoModeMap() {["Off":"OFF","On":"ON"]}
def static humidifyOscillationModeMap() {["DEGREE_45":"0045","DEGREE_90":"0090","BREEZE":"BRZE","CUST":"CUST"]}
def static waterHardnessValueMap() {["Soft":"2025","Medium":"1350","Hard":"0675"]}


metadata {
    definition (name: 'Dyson Pure Humidify Cool Driver',
            namespace: 'dpasirst',
            author: 'Dave Pasirstein',
            importUrl: 'https://raw.githubusercontent.com/dpasirst/hubitat/main/drivers/dyson/dyson-pure-humidify-driver.groovy') {
        capability "Actuator"
        capability 'Sensor'
        capability "Polling"
        ...
        attribute 'oscillationMode', 'enum', humidifyOscillationModeMap().values().toList() //oson
        attribute 'humidificationMode', 'enum', humidificationModeMap().values().toList() //hume
        attribute 'humidificationAutoMode', 'enum', humidificationAutoModeMap().values().toList() //hume
        attribute 'targetHumidityPct', 'string' //humt
        attribute 'targetHumidityPctFriendly', 'number' //friendly humt
        attribute 'autoTargetHumidityPct', 'string' //rect
        attribute 'autoTargetHumidityPctFriendly', 'number' //friendly rect
        attribute 'waterHardness', 'enum', waterHardnessValueMap().values().toList() //wath
        attribute 'timeUntilNextDeepCleaningHrs', 'number' //cltr
        attribute 'timeUntilCleaningCompletesMinutes', 'number' //cltr
        ...
        command 'setOscillationMode',[[name: "Change Mode", type:"ENUM",
                                constraints: oscillationMap().keySet().toList()],
                                      [name: "Oscillation Mode", type:"ENUM",
                                       constraints: humidifyOscillationModeMap().keySet().toList()]]
        ...
        //humid
        command 'setHumidifyOscillationMode',[[name: "Change Mode", type:"ENUM",
                                          constraints: humidifyOscillationModeMap().keySet().toList()]]
        command 'setHumidificationMode',[[name: "Change Mode", type:"ENUM",
                                          constraints: humidificationModeMap().keySet().toList()]]
        command 'setHumidificationAutoMode',[[name: "Change Mode", type:"ENUM",
                                          constraints: humidificationAutoModeMap().keySet().toList()]]
        command 'setTargetHumidity',[[name: "Target Number*",type:"NUMBER",
                                      description: "Set target humidity"]]
        command 'setWaterHardness', [[name: "Change Mode", type:"ENUM",
                                      constraints: waterHardnessValueMap().keySet().toList()]]
    }
...
private static DYSON_PARAM_COOL_STATE_MAP()  {
    [
        ...
        'oscillation': "oscs", //pcbase
        'oscillationMode':"ancp", //h
        'humidificationMode':"hume", //h
        'humidificationAutoMode':"haut", //h
        'targetHumidityPct':"humt", //h
        'autoTargetHumidityPct':"rect", //h
        'waterHardness' : "wath", //h
        'timeUntilNextDeepCleaningHrs': "cltr", //h
        'timeUntilCleaningCompletesMinutes':"cdrr", //h
        'nightMode': "nmod", //base
        'fanSpeed': "fnsp", //base
        'fanState': "fnst", //base
...
//command
def setOscillationMode(mode, humidifyOscillationMode) {
    ...
    if (off) {
        ...
        String om = humidifyOscillationModeMap().getOrDefault(humidifyOscillationMode,device.currentValue("oscillationMode")?.toString())
        if (om == null) {
            om = humidifyOscillationModeMap().DEGREE_45
        }
        //we now try to set the new value
        def config = [(DYSON_PARAM_COOL_STATE_MAP().oscillation): newMode,
                      (DYSON_PARAM_COOL_STATE_MAP().fanPower):fanPowerMode().On,
                      (DYSON_PARAM_COOL_STATE_MAP().oscillationMode):om]
        log.info("DYSON Set Oscillation Mode: ${config}")
        setConfiguration(config)
    }
}
...
//command
def setHumidifyOscillationMode(mode) {
    def newMode = humidifyOscillationModeMap().getOrDefault(mode,humidifyOscillationModeMap().DEGREE_45)
    def config = [(DYSON_PARAM_COOL_STATE_MAP().oscillationMode): newMode]
    log.info("DYSON Set Humidify Oscillation Mode: ${config}")
    setConfiguration(config)
}
//command
def setHumidificationMode(mode) {
    def newMode = humidificationModeMap().getOrDefault(mode,humidificationModeMap().Off)
    def config = [(DYSON_PARAM_COOL_STATE_MAP().humidificationMode): newMode]
    log.info("DYSON Set Humidification Mode: ${config}")
    setConfiguration(config)
}
//command
def setHumidificationAutoMode(mode) {
    def newMode = humidificationAutoModeMap().getOrDefault(mode,humidificationAutoModeMap().Off)
    def config = [(DYSON_PARAM_COOL_STATE_MAP().humidificationAutoMode): newMode]
    log.info("DYSON Set Humidification Auto Mode: ${config}")
    setConfiguration(config)
}
//command
def setTargetHumidity(mode) {
    def target = mode?.toString()?.toInteger()
    if (target == null || target < 1 || target > 100) {
        target = device.currentValue("targetHumidityPct")?.toString()?.toInteger()
        if (target == null || target < 1 || target > 100) {
            target = 30
        }
    }
    def config = [(DYSON_PARAM_COOL_STATE_MAP().targetHumidityPct): sprintf("%04d",target),
                  (DYSON_PARAM_COOL_STATE_MAP().humidificationAutoMode):humidificationAutoModeMap().Off]
    log.info("DYSON Set Humidification Auto Mode: ${config}")
    setConfiguration(config)
}
//command
def setWaterHardness(mode) {
    def newMode = waterHardnessValueMap().getOrDefault(mode,null)
    if (newMode == null) {
        log.error("DYSON Cannot Set Water Hardness, invalid value (${mode})")
        return
    }
    def config = [(DYSON_PARAM_COOL_STATE_MAP().waterHardness): newMode]
    log.info("DYSON Set Water Hardness Mode: ${config}")
    setConfiguration(config)
}
